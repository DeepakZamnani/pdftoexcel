...
import { Upload, FileText, Download, Loader2, RefreshCcw, AlertCircle, CheckCircle2, ChevronRight, Layers, FileSearch, Sparkles, Lock, X } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Process normal file (not password protected)
      processFile(file);
    }
  };
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      // Unlock the PDF with the password
      const unlockedFile = await unlockPDF(originalFile, state.password);
      
      // Close dialog and process the unlocked file
      setState(prev => ({
        ...prev,
        showPasswordDialog: false,
        password: '',
